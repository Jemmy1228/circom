...
use circom_algebra::algebra::{ArithmeticExpression, PureArithmeticExpression, HintExpression};
...
fn argument_into_slice(argument: &Argument) -> AExpressionSlice {
    use PureArithmeticExpression::Number;
    let arithmetic_expressions: Vec<ArithmeticExpression<String>> =
        argument.values.iter().map(|v|  ArithmeticExpression{
            pure: Number { value: v.clone() },
            hint: HintExpression::Number { value: v.clone() },
        }).collect();
...
fn treat_statement(stmt: &mut Statement, context: &Context, reports: &mut ReportCollection, flags: FlagsExecution, prime: &String) {
    if stmt.is_initialization_block() {
        ...
        treat_block(stmt, context, reports, flags, prime)
